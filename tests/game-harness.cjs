...
function setup(){
 ...
 const sandbox={Image,Audio,console,Math,innerWidth:480,innerHeight:800,document:{querySelector:s=>s==='#g'?canvas:s==='#bomb-control'?bomb:music,createElement:()=>({...canvas})},addEventListener:window.addEventListener.bind(window),requestAnimationFrame(){}};
 vm.createContext(sandbox);
 for(const file of ['aircraft.js','pickups.js','mobile-controls.js'])vm.runInContext(fs.readFileSync(path.join(root,file),'utf8'),sandbox);
